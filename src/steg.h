...
/** A steganography instance must define a private subclass of this
    ...
    module name in STEG_DEFINE_MODULE, and should be declared into the
...
#define STEG_DEFINE_MODULE(name_, csm, scm, mcci, mci)  \
  /* detect and new_ dispatchers */                     \
  static bool name_##_detect(conn_t *conn)              \
  { return name_::detect(conn); }                       \
  static steg_t *name_##_new(bool is_clientside)        \
  { steg_t *s = new name_;                              \
    ...
    return s;                                           \
                                                        ...
                                                        \
  /* module object */                                   \
  extern const steg_module s_mod_##name_ = {            \
    #name_, csm, scm, mcci, mci,                        \
    name_##_detect, name_##_new                         \
  } /* deliberate absence of semicolon */

#define STEG_DECLARE_METHODS(name)                              \
