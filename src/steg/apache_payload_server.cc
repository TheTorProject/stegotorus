...
{
  int r, i, cnt, found = 0, numCandidate = 0, current;
  ...
  for (itr_payloads = _payload_database.payloads.begin(), itr_best = _payload_database.payloads.begin(); itr_payloads != _payload_database.payloads.end() && numCandidate < MAX_CANDIDATE_PAYLOADS; itr_payloads++) {
    ...
    if ((*itr_payloads).second.capacity <= (unsigned int)cap || (*itr_payloads).second.type != contentType)
      continue;
    ...
    if ((*itr_best).second.length > (*itr_payloads).second.length ||
        (*itr_best).second.type != contentType)
  ...
  if (found)
    ...
    log_debug("cur payload size=%d, best payload size=%d, num candidate=%d\n",
              ...
              numCandidate);
    fetch_url_raw((*itr_best).second.url);
    *size = (*itr_best).second.length;
    return 1;
  } else {
    return 0;
  }
...
pair<unsigned long, char*> ApachePayloadServer::fetch_url(string url, unsigned long payload_length)
{
  CURLcode res;
  ...
  if (payload_length > c_max_buffer_size)
    {
      ...
      return (pair<unsigned long, char*>)(0,NULL);
